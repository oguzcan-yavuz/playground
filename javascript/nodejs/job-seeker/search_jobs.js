...
async function get_company_ids() {
    let company_urls = [];
    let page_token = "";
            ...
            .then(function(response) {
                ...
                for(let i in results) {
                    if(results.hasOwnProperty(i)) {
                        get_company_urls(results[i].place_id).then((result) => {
...
async function get_company_urls(company_id) {
...
get_company_ids().then((company_urls) => {
