...
def string_creator(string, generations, rules):
    if generations == 0:
        return string
    string = string[-1] + string + string[0]
    recursive_string = ""
    for j in range(0, len(string) - 1):
        for node in rules:
            if node[0] == string[j:j+3]:
                recursive_string += node[1][0]
                break
    return string_creator(recursive_string, generations - 1, rules)


def cellular_automaton(string, number, generations):
    rule = [['xxx', ['x', 128]], ['xx.', ['.', 64]], ['x.x', ['.', 32]], ['x..', ['.', 16]],
            ['.xx', ['x', 8]], ['.x.', ['x', 4]], ['..x', ['x', 2]], ['...', ['.', 1]]]
    # ['...', ['.', 1]]
    for node in rule:
        if number >= node[1][1]:
            node[1][0] = 'x'
            ...
            node[1][0] = '.'
    return string_creator(string, generations, rule)
