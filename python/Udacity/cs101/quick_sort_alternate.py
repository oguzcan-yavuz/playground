    ...
    if not pages or len(pages) <= 1:
        ...
        for page in pages[1:]:
            if rank[page] <= pivot:
                ...
                right.append(page)
        return quick_sorted(left, rank) + [pages[0]] + quick_sorted(right, rank)


def highest_five(arr):
    if arr is not None:
        if len(arr) < 5:
            lenght = len(arr)
        else:
...
def ordered_search(index, ranks, keyword):
    ...
    return highest_five(quick_sorted(pages, ranks))
