...
def date_converter(language, date):
    slash = date.find('/')
    while slash is not -1:
        date = date[:slash] + " " + date[slash + 1:]
        slash = date.find('/', slash + 1)
    # ['5', '11', '2012']
    numeric = date.split()
    month = language[int(numeric[0])]
    result = numeric[1] + ' ' + month + ' ' + numeric[2]
    return result

def better_converter(language, date):
